...
export const editComment = async (req, res) => {
    ...
    try {
        ...
        if (comment.user_id !== user_id) {
            return res.status(403).json({ msg: "Anda tidak diizinkan untuk mengedit komentar ini" });
        }
...
export const deleteComment = async (req, res) => {
    ...
    try {
        ...
        if (comment.user_id !== user_id) {
            return res.status(403).json({ msg: "Anda tidak diizinkan untuk menghapus komentar ini" });
        }
...
export const viewComments = async (req, res) => {
    try {
        const response = await Comments.findAll({
            attributes: ['comment_id, comment_text', 'user_id,', 'news_id'] // Hanya mengambil atribut 'comment'
        });
...
// Fungsi untuk mencari komentar berdasarkan user_id dan news_id
