...
export const Register = async (req, res) => {
    ...
    try {
        ...
        // Validasi Role
        const validRoles = ['user', 'author']; // Definisikan peran yang valid
        ...
        if (role === 'author') {
            return res.status(201).json({ msg: "Registrasi sebagai author berhasil, menunggu validasi admin" });
        }
...
export const ValidateAuthor = async (req, res) => {
    ...
    try {
        ...
        if (!user || user.status !== 'pending') {
            return res.status(404).json({ msg: "Pengguna tidak ditemukan atau tidak membutuhkan validasi" });
        }
...
export const Login = async (req, res) => {
    ...
    try {
        ...
        if (user.role === 'author' && user.status !== 'active') {
            return res.status(403).json({ msg: "Akun belum divalidasi oleh admin" });
        }


        const user_id = user.user_id;
        const username = user.username;
        const userEmail = user.email; // Move the initialization here
        const role = user.role;

        const accessToken = jwt.sign({ user_id, username, userEmail, role }, process.env.ACCESS_TOKEN_SECRET, {
            expiresIn: '1h'
        });
...
export const forgetPassword = async (req, res) => {
    ...
    try {
        ...
        const resetToken = jwt.sign({ user_id: user.user_id }, process.env.RESET_TOKEN_SECRET, { expiresIn: '1h' });
