...
export const addNews = async (req, res) => {
    ...
    try {
        ...
        if (!author || author.role !== 'author') {
            return res.status(403).json({ msg: "Hanya author yang dapat menambahkan berita" });
        }
        ...
        res.status(201).json({ msg: "Berita berhasil ditambahkan, menunggu verifikasi admin" });
    } catch (error) {
...
export const verifyNews = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ msg: "Berita berhasil diverifikasi dan dipublikasikan" });
    } catch (error) {
        res.status(500).json({ msg: error.message });
    }
};
